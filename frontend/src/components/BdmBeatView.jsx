...
import { Search, Building2, AlertCircle, TrendingUp, ChevronRight, Download } from 'lucide-react';
...
export default function BdmBeatView({
  ...
  const handleExportCSV = () => {
    const headers = {
      ...
      last_units: 'Last Month Units',
      last_val: 'Last Month Value (INR)',
      owner: 'Owner / Contact Person'
    };
    exportToCSV(`Beat_Plan_${territory}_${new Date().toISOString().slice(0,10)}`, filteredBeat, headers);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 2. Fast Search Bar & CSV Export Action */}
      <div className="flex flex-col sm:flex-row items-center gap-3">
        ...
        <button
          ...
          className="w-full sm:w-auto px-4 py-3 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold rounded-xl border border-slate-700 transition flex items-center justify-center gap-2 shadow-sm whitespace-nowrap">
          <Download className="w-4 h-4 text-blue-400" />
          Export Beat CSV ({filteredBeat.length})
        </button>
      </div>
